...
import { addProprietary, deleteProprietary } from 'src/store/apps/condominium/residentialDetail'
...
interface ProprietaryActionsDialogInterface {
...
type ProprietaryData = (LegalPerson | NaturalPerson) & {
...
const defaultValues: ProprietaryData = {
    id: '0',
    name: '',
    date_of_birth: '',
    email: '',
...
const actionTypeConfig = {
    add: {
        ...
        content: 'Insira um proprietário nesta Unidade Residencial.'
    },
    edit: {
        title: 'Editar Proprietário',
        content: 'Edite as informações de um proprietário dessa Unidade Residencial.'
    },
    delete: {
        title: 'Remover Proprietário',
        content: 'Remova o proprietário da Unidade Residencial.'
    },
    view: {
        title: 'Visualizar Proprietário',
        content: 'Visualize as informações de um proprietário dessa Unidade Residencial'
    },
    default: {
...
const ProprietaryActionsDialog = ({
    actionType,
    actionData,
    open,
    toggle,
    residentialId
}: ProprietaryActionsDialogInterface) => {
    ...
    const searchCPFOrCNPJ = async () => {
        ...
        const { data } =
            await PeopleService.get_person_and_active_uhab_user_role_by_uhab_user_role_and_person_identification.get({
                ...
                fk_lookup_type_of_uhab_user_role: EnumUhabUserRole.Proprietary,
    ...
    const _onSubmit = async (data: ProprietaryData): Promise<void> => {
        const commonPayload = {
            id: data.id,
            name: data.name,
        ...
        const actionDispatchMap = {
            add: () => wrapDispatch(addProprietary(commonPayload)),
            //edit: () => wrapDispatch(updatePropertyOrResident(commonPayload)),
            delete: () =>
                wrapDispatch(
                    deleteProprietary({
                        id: commonPayload.id,
        ...
        const dispatchAction = (actionDispatchMap as any)[actionType]
        ...
        queryClient.invalidateQueries({
            queryKey: ['residential.proprietaries', residentialId]
        })
                ...
                <Box sx={{ mb: 8 }}>
                    {actionType === 'delete' && (
                        <Alert severity='warning'>
                            ...
                            {`Esta ação irá excluir permanentemente o proprietário. Por favor, verifique se está absolutamente certo disso antes de proceder!`}
                        </Alert>
                    )}
                    {disableSaveButton && (
                        <Alert severity='error'>
                            <AlertTitle>Atenção!</AlertTitle>
                            {`Não é possível adicionar um proprietário que já existe. Esse ${
...
export default ProprietaryActionsDialog
