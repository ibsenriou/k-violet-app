...
import {
    ...
    updateRenter,
    deleteRenter,
} from 'src/store/apps/condominium/commercialDetail'
...
interface RenterActionsDialogInterface {
...
type RenterData = (LegalPerson | NaturalPerson) & {
...
const defaultValues: RenterData = {
    id: '0',
    name: '',
    date_of_birth: '',
    email: '',
...
const actionTypeConfig = {
    add: {
        title: 'Adicionar Locatário Comercial',
        content: 'Insira um locatário nesta Unidade Comercial.'
    },
    edit: {
        title: 'Editar Locatário Comercial',
        content: 'Edite as informações de um locatário dessa Unidade Comercial.'
    },
    delete: {
        title: 'Remover Locatário Comercial',
        content: 'Remova o locatário da Unidade Comercial.'
    },
    view: {
        title: 'Visualizar Locatário Comercial',
        content: 'Visualize as informações de um locatário dessa Unidade Comercial'
    },
    default: {
...
const RenterActionsDialog = ({ actionType, actionData, open, toggle, commercialId }: RenterActionsDialogInterface) => {
    const {
        reset,
        control,
        handleSubmit,
    ...
    const searchCPFOrCNPJ = async () => {
        ...
        const { data } =
            await PeopleService.get_person_and_active_uhab_user_role_by_uhab_user_role_and_person_identification.get({
                ...
                fk_lookup_type_of_uhab_user_role: EnumUhabUserRole.Renter,
                fk_uhab: commercialId
            })
    ...
    const _onSubmit = async (data: RenterData): Promise<void> => {
        const commonPayload = {
            id: data.id,
            name: data.name,
        ...
        const actionDispatchMap = {
            add: () => wrapDispatch(addRenter(commonPayload)),
            edit: () => wrapDispatch(updateRenter(commonPayload)),
            delete: () => wrapDispatch(deleteRenter({id: commonPayload.id.toString()})),
            view: () => {} // do nothing
        }
        ...
        queryClient.invalidateQueries({
          queryKey: ['commercial.renters', commercialId]
        })
    ...
    useEffect(() => {
        async function setUhabMainRole() {
            if (actionData?.id && EnumUhabUserRole?.Renter) {
                const uhabUserRoles = (
        ...
        setUhabMainRole()
    }, [open, actionData, commercialId, EnumUhabUserRole, setValue])
                ...
                <Box sx={{ mb: 8 }}>
                    {actionType === 'delete' && (
                        <Alert severity='warning'>
                            ...
                            {`Esta ação irá excluir permanentemente o locatário comercial. Por favor, verifique se está absolutamente certo disso antes de proceder!`}
                        </Alert>
                    )}
                    {disableSaveButton && (
                        <Alert severity='error'>
                            <AlertTitle>Atenção!</AlertTitle>
                            {`Não é possível adicionar um locatário que já existe. Esse ${
                    ...
                    <Grid item sm={6} xs={12}>
                        <FormControl fullWidth style={{ alignItems: 'flex-start' }}>
                            <Controller
                                ...
                                rules={{ required: true }}
                                render={({ field: { value, onChange } }) => (
                                    <Tooltip title='Locatário que receberá as informações e comunicados'>
                                        <FormControlLabel
                                            label='Responsável'
...
export default RenterActionsDialog
