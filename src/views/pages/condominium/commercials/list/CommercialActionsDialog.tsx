...
import { addCommercial, deleteCommercial, updateCommercial } from 'src/store/apps/condominium/commercials'
...
interface CommercialActionsDialogInterface {
    actionType: 'add' | 'edit' | 'delete' | null
    actionData: CommercialType | {}
    open: boolean
    toggle: () => void
}
...
const schema = yup.object().shape({
    ...
    fk_lookup_type_of_commercial: yup.string().required('Este campo é obrigatório!')
})
...
const CommercialActionsDialog = ({ actionType, actionData, open, toggle }: CommercialActionsDialogInterface) => {
    const commercialsStore = useSelector((state: RootState) => state.commercials)
    ...
    const actionTypeConfig = {
        add: {
            ...
            content: 'Insira uma Unidade Comercial neste Condomínio.'
        },
        edit: {
            title: 'Editar Comercial',
            content: 'Edite as informações da Unidade Comercial.'
        },
        delete: {
            title: 'Remover Comercial',
            content: 'Remova a Unidade Comercial.'
        },
        default: {
    ...
    const _onSubmit = async (data: CommercialType): Promise<void> => {
        const commonPayload = {
            id: data.id,
            description: '',
            name: data.name,
        ...
        const actionDispatchMap = {
            add: () => wrapDispatch(addCommercial({ ...commonPayload, id: '0' })),
            edit: () => wrapDispatch(updateCommercial(commonPayload)),
            delete: () => wrapDispatch(deleteCommercial({ id: data.id }))
        }
                ...
                <Box sx={{ mb: 8 }}>
                    {actionType === 'delete' && (
                        <Alert severity='warning'>
                            ...
                            {`Esta ação irá excluir este comercial do Condomínio e poderá influenciar em documentações em diversos módulos do sistema!\n
                ...
                <Grid container spacing={6}>
                    ...
                    <Grid item sm={12} xs={12}>
                        <FormControl fullWidth sx={{ mb: 3 }}>
                            <InputLabel id='fk_lookup_type_of_commercial'>Tipo de Comercial</InputLabel>
                            <Controller
                                name='fk_lookup_type_of_commercial'
                                control={control}
                                rules={{ required: true }}
                                render={({ field: { value, onChange } }) => (
                                    <Select
                                        fullWidth
                                        value={value}
                                        id='fk_lookup_type_of_commercial'
                                        label='Tipo de Comercial'
                                        labelId='select-the-commercial-type'
                                        onChange={e => onChange(e.target.value)}
                                        disabled={actionType === 'delete'}
                                    >
                                        {commercialsStore.fk_lookup_type_of_commercial.map(({ id, description }) => (
                            ...
                            />
                            {errors.fk_lookup_type_of_commercial && (
                                <FormHelperText sx={{ color: 'error.main' }}>
                                    {errors.fk_lookup_type_of_commercial.message}
                                </FormHelperText>
                            )}
                        </FormControl>
                    </Grid>
                            ...
                            <Controller
                                ...
                                render={({ field: { value, onChange } }) => (
                                    <Select
                                        ...
                                        disabled={actionType === 'delete'}
                                    >
                                        {commercialsStore.condominiumGroupings.map(({ id, name }) => (
                                            <MenuItem key={id} value={id}>
                                                {name}
                                            </MenuItem>
                                        ))}
...
export default CommercialActionsDialog
